...
Detections2D run_yolo(ImageData& data) {
	...
	static torch::jit::script::Module yolo_model = []() {
		try {
			yolo_model = torch::jit::load(std::filesystem::path(CMAKE_SOURCE_DIR) / "data" / "yolo" / (std::to_string(height) + 'x' + std::to_string(width)) / "yolov9c.torchscript", device);
			yolo_model.eval();
