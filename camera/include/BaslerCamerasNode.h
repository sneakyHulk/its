...
class BaslerCamerasNode : public Pusher<ImageDataRaw> {
   public:
	...
	explicit BaslerCamerasNode(std::map<std::string, MacIPAddressConfig>&& camera_name_mac_ip_address) {
		auto index = 0;
		for (auto const& [cam_name, mac_ip_address] : camera_name_mac_ip_address) {
			_camera_name_mac_address_index_map.emplace(cam_name, mac_ip_address.mac, index++);
		}
