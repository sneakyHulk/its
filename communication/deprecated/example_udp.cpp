...
class ccRTP_Hello_Rx {
   ...
   public:
	ccRTP_Hello_Rx() {
		...
		// SSRC identifier
		socket = new RTPSession(local_ip);
		ssrc = socket->getLocalSSRC();
	}

	~ccRTP_Hello_Rx() {
		cout << endl << "Destroying receiver -ID: " << hex << (int)ssrc;
		terminate();
		delete socket;
		cout << "... " << "destroyed.";
	}
	...
	void operator()() {
		cout << "Hello, " << defaultApplication().getSDESItem(SDESItemTypeCNAME) << " ..." << endl;
		...
		if (!socket->addDestination(local_ip)) cerr << "Rx (" << hex << (int)ssrc << "): could not connect to port." << TRANSMITTER_BASE;

		cout << "Rx (" << hex << (int)ssrc << "): " << local_ip.getHostname() << " is waiting for salutes in port " << RECEIVER_BASE << "..." << endl;

		socket->setPayloadFormat(StaticPayloadFormat(sptJPEG));
		socket->startRunning();
		// Let's check the queues  (you should read the documentation
		// so that you know what the queues are for).
		cout << "Rx (" << hex << (int)ssrc << "): The queue is " << (socket->isActive() ? "" : "in") << "active." << endl;
		...
		for (int i = 0; true; i++) {
			...
			cout << "Rx (" << hex << (int)ssrc << "): [receiving at " << tmstring << "]: " << adu->getData() << endl;
			delete adu;
		}
	}
};
...
class ccRTP_Hello_Tx {
   ...
   public:
	...
	~ccRTP_Hello_Tx() {
		cout << endl << "Destroying transmitter -ID: " << hex << (int)ssrc;
		terminate();
		delete socket;
		cout << "... " << "destroyed.";
	}
	...
	void operator()() {
		// redefined from Thread.
		cout << "Tx (" << hex << (int)ssrc << "): " << local_ip.getHostname() << " is going to salute perself through " << local_ip << "..." << endl;

		// Set up connection
		socket->setSchedulingTimeout(20000);
		socket->setExpireTimeout(3000000);

		if (!socket->addDestination(local_ip)) cerr << "Tx (" << hex << (int)ssrc << "): could not connect to port." << RECEIVER_BASE;

		cout << "Tx (" << hex << (int)ssrc << "): Transmitting salutes to port " << RECEIVER_BASE << "..." << endl;
		...
		// Let's check the queues  (you should read the documentation
		// so that you know what the queues are for).
		cout << "Tx (" << hex << (int)ssrc << "): The queue is " << (socket->isActive() ? "" : "in") << "active." << endl;
		...
		for (int i = 0; true; i++) {
			...
			cout << "Tx (" << hex << (int)ssrc << "): sending salute " << "no " << dec << i << ", at " << tmstring << "..." << endl;
...
int main(int argc, char *argv[]) {
	...
	ccRTP_Hello_Rx receiver;
	ccRTP_Hello_Tx transmitter;

	cout << "This is rtphello, a very simple test program for ccRTP." << endl << "Strike [Enter] when you are fed up with it." << endl;

	// Start execution of hello now.

	std::thread receiver_thread(&ccRTP_Hello_Rx::operator(), &receiver);
	std::thread transmitter_thread(&ccRTP_Hello_Tx::operator(), &transmitter);
