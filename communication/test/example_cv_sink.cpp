...
int main(int argc, char **argv) {
	...
	string pipeStr = "filesrc location=" + fileName + " ! decodebin ! videoconvert ! appsink name=mysink max-buffers=2 sync=1 caps=video/x-raw,format=BGR";
	GError *err = nullptr;
	...
	thread threadBus([&data]() -> void { codeThreadBus(data.pipeline, data, "GOBLIN"); });
