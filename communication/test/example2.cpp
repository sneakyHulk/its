...
class ccRTP_Hello_Rx : public Thread {
   ...
   public:
	...
	void run(void) {
		...
		if (!socket->addDestination(local_ip, TRANSMITTER_BASE)) cerr << "Rx (" << hex << (int)ssrc << "): could not connect to port." << TRANSMITTER_BASE;
...
class ccRTP_Hello_Tx : public Thread, public TimerPort {
   ...
   public:
	...
	void run(void) {
		...
		socket->setSchedulingTimeout(20000);
		socket->setExpireTimeout(3000000);
		if (!socket->addDestination(local_ip, RECEIVER_BASE)) cerr << "Tx (" << hex << (int)ssrc << "): could not connect to port." << RECEIVER_BASE;
