...
void codeThreadSrcV(GoblinData &data) {
	...
	for (;;) {
		...
		video.read(frame);
		if (frame.empty()) {
			video = VideoCapture(data.fileName);
			video.read(frame);
		}
...
int main(int argc, char **argv) {
	...
	string pipeStr = "appsrc name=mysrc format=time caps=video/x-raw,format=BGR ! videoconvert ! autovideosink";  // autovideosink"; // sync=1";
	GError *err = nullptr;
	...
	// thread threadBus([&data]() -> void { codeThreadBus(data.pipeline, data, "ELF"); });

	// Start the appsrc process thread
	thread threadSrcV([&data]() -> void { codeThreadSrcV(data); });
